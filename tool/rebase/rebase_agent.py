...
async def main():
    ...
    system_instructions = (
        ...
        "A git rebase of `features/subprofiles-mobile` (our branch implementing persona subprofiles, profile sheets, and persona mentions) against `upstream/canary` "
        ...
        "- run_command: Run shell commands (e.g. `git status`, `git diff`, `git log`, `flutter test`, `git add <file>`, `git rebase --continue`)\n"
        ...
        "OBJECTIVES & RULES:\n"
        "1. PRESERVE ALL SUBPROFILE FEATURES: Keep all persona models, Riverpod providers (persona_providers.dart, public_persona_provider.dart), profile sheets (user_profile_sheet.dart), composer autocomplete (composer_autocomplete_field.dart, composer_mention_controller.dart), mention widgets (message_mention.dart), native markdown bindings, and tests.\n"
        ...
        "   - Persona unit & widget tests: `flutter test test/features/chat/services/composer_mention_controller_test.dart test/features/chat/presentation/widgets/messages/message_mention_test.dart test/features/profile/persona_test.dart test/features/profile/persona_matcher_test.dart test/features/profile/persona_providers_test.dart test/features/chat/presentation/widgets/composer/typing_indicator_bar_test.dart test/features/chat/utils/messages/message_grouping_utils_test.dart`\n"
        ...
        "   - Code analysis: `flutter analyze --no-fatal-warnings --no-fatal-infos`\n"
        ...
        "   If you make any file edits or code fixes after the rebase finishes to resolve test failures, linter/analyzer warnings, or compiler errors:\n"
        "   - You MUST stage and commit those changes with a descriptive conventional commit message (e.g. `git commit -am 'fix(rebase): resolve analyzer error in unified_push_incoming_policy'`).\n"
    ...
    prompt = (
        ...
        f"resolve the conflicts, stage them with git add, continue the rebase, and run the flutter and dart test suites along with `flutter analyze --no-fatal-warnings --no-fatal-infos`.\n"
